...
本地文件位于 `app/src/main/res/drawable-nodpi/`。从图片 CDN 获取 sRGB JPEG，保留比例，最长边缩至 1600 像素，JPEG 质量 87，去除 EXIF；共约 1.7 MiB。仅做尺寸和编码处理，未额外调色。示例原片可能已经由摄影师调色，不代表未经处理的相机原片。

入口：相机页打开相册 → “内置参考照片”；未授予相机权限时可直接点击“先用内置参考照片”。选择“用这张调色”通过 `android.resource` URI 进入统一编辑器，无需相册权限，也不会复制或覆盖 APK 内资源。浏览和本地调色无需网络，AI 分析仍需点击并联网。
